...
class Tests(unittest.TestCase):
    ...
    def test_one_root(self):
        self.assertEqual(regula_falsi(lambda x: x ** 3 + 2 * x ** 2 + 10 * x - 20, 1, 2, 0), 1.3688081078213727)

    # Failed - bring incorrect root
    def test_not_correct_root(self):
        self.assertEqual(regula_falsi(lambda x: x**2, 1, 2, 0), 1)
    ...
    def test_other_root(self):
        self.assertEqual(regula_falsi(lambda x: x**5 - 2 * x + 1, -1.4, -1.28, 1.6), -1.2887431330691683)
